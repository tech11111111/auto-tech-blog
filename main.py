...
def publish_to_devto(topic_info, content, image_url=None):
    ...
    for tag in raw_tags[:4]:
        cleaned = re.sub(r"[^a-z0-9]", "", tag.lower())[:30]
        if cleaned:
            clean_tags.append(cleaned)
    if not clean_tags:
        clean_tags = ["tech"]

    description = f"{topic_info.get('focus_keyword', '')} — {topic_info['blog_title']}"[:255]
    ...
    try:
        ...
        if r.status_code in (200, 201):
            ...
            raise Exception(f"Dev.to error: {r.status_code} — {r.text[:200]}")
    except Exception as e:
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 7 — Save to Memory & Update Brain
# ─────────────────────────────────────────────────────────────────────────────
def save_post(memory, brain, topic_info, url, article_id, content):
    """Save post to memory and update brain stats"""
    try:
        post_record = {
            "date":         TODAY,
            "title":        topic_info["blog_title"],
            ...
            "industry":     topic_info.get("industry", "CoreTech"),
            "length":       topic_info.get("estimated_length", "medium"),
            "word_count":   len(content.split()),
            "viral_score":  topic_info.get("viral_score", 0),
            "seo_score":    topic_info.get("seo_score", 0),
            "views":        0,
            "likes":        0,
            "comments":     0,
            "performance":  "pending"
        ...
        # Also save to brain's posts_history
        brain.setdefault("posts_history", [])
        brain["posts_history"].append({
            "title":    topic_info["blog_title"],
            "date":     TODAY,
            "url":      url,
            "industry": topic_info.get("industry", "CoreTech")
        })
        brain["posts_history"] = brain["posts_history"][-50:]

        # Update identity stats
        brain["identity"]["total_posts"] = len(memory["posts"])
        save_brain(brain)
        print("  ✅ Saved to memory & brain")
    except Exception as e:
...
def main():
    print("\n" + "="*60)
    print(f"  🤖 TechPulse AI AutoBlog — {TODAY}")
    print("="*60 + "\n")
    ...
    print(f"  📚 Lessons loaded: {lessons}")
    print(f"  📝 Past posts: {total_posts}")
    print(f"  🔥 Viral predictions: {'✅' if brain.get('viral_predictions') else '❌'}")
    print(f"  🌍 Industry intel: {'✅' if brain.get('industry_intelligence') else '❌'}")
    print(f"  🔎 SEO intel: {'✅' if brain.get('seo_intelligence') else '❌'}\n")
    ...
    save_post(memory, brain, topic_info, url, aid, content)

    print("\n" + "="*60)
    print("  🎉 SUCCESS!")
    print(f"  📝 {topic_info['blog_title']}")
    print(f"  🌍 Industry: {topic_info.get('industry', 'N/A')}")
    print(f"  🔥 Viral score: {topic_info.get('viral_score', 'N/A')}")
    print(f"  🔗 {url}")
    print(f"  📊 Total posts: {total_posts + 1}")
    print("="*60 + "\n")
